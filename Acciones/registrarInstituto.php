...
$user = new Usuario(null, $nombre, $apellido, $correo, $password, '2');
...
if ($resultado->fetch_assoc()['count(*)'] == '0') {
	...
	$consulta = sprintf(
		"SELECT count(*) FROM tblInstituto WHERE CodigoIns = '%s'",
	...
	if ($resultado->fetch_assoc()['count(*)'] == '0') {
		...
		$consulta = sprintf(
			"INSERT INTO tblInstituto(CodigoIns, NombreIns, Pase, IDMunicipio, Direccion, Director) values('%s','%s','%s','%s','%s','%s')",
			$conexion->antiInyeccion($codIns),
		...
		$consulta = sprintf(
			"INSERT INTO tblPlan(IDTipoPlan, IDInstituto, DiasPrueba, AulasDisponibles) values('%s','%s','%s','%s')",
