...
if ($data['count(*)'] == '1') {
	...
	if ($password == $data['password']) {
		#	Crear Sesion
		$consulta = sprintf(
			"SELECT idusuario, correo, tipousuario, nombre, apellido, imagen FROM tblUsuario WHERE correo = '%s'",
		...
		$consulta = sprintf(
			"SELECT color FROM tblcolor INNER JOIN tbltema ON tbltema.IDColor = tblcolor.IDColor WHERE tbltema.IDUsuario = '%s'",
			$conexion->antiInyeccion($_SESSION['ID'])
		);
